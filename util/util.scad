 ...
 */
function UTL_tail(vector) = UTL_empty(vector)
                                ? undef
                                : len(vector) == 1
                                      ? []
                                      : [for (i = [1:len(vector) - 1])
                                                vector[i]];
 ...
 */
function UTL_last(vector) = UTL_empty(vector)
                                ? undef
 ...
 */
function UTL_reverse(vector) = UTL_empty(vector)
                                   ? []
                                   : concat(UTL_reverse(UTL_tail(vector)),
                                            UTL_head(vector));
...
function UTL_equal(value_a, value_b) =
    ...
    (!UTL_iterable(value_a) && UTL_iterable(value_b))
        ? false
        :
        ...
        (UTL_iterable(value_a) && !UTL_iterable(value_b))
            ...
            (!UTL_iterable(value_a) && !UTL_iterable(value_b))
                ...
                :
                // Both iterables
                (UTL_empty(value_a) && UTL_empty(value_b))
                    ? true
                    :
                    ...
                    (UTL_empty(value_a) || UTL_empty(value_b))
                        ...
                        (!UTL_equal(UTL_head(value_a), UTL_head(value_b)))
                            ? false
                            :
 ...
 */
function UTL_all(vector) = (len(vector) == 0) ? true
                                              : (!UTL_head(vector))
                                                    ? false
 ...
 */
function UTL_any(vector) = (len(vector) == 0)
                               ? false
 ...
 */
function UTL_contains(vector,
                      value) = (len(vector) == 0)
                                   ? false
                                   : UTL_equal(UTL_head(vector), value)
                                         ? true
                                         : UTL_contains(UTL_tail(vector),
                                                        value);
...
function UTL_zip(matrix) =
    len(matrix) == 0
        ...
        : (min([for (i = matrix) len(i)]) == 0)
              ? []
              : let(remainder = [for (i = [0:len(matrix) - 1])
                            UTL_tail(matrix[i])],
...
function UTL_sort(vector) =
    UTL_empty(vector) ? []
                      : let(pivot = vector[floor(len(vector) / 2)],
...
function UTL_onePoleFilter(vector, coefficient, previous = undef) =
    UTL_empty(vector)
        ? []
        : (previous == undef)
              ? UTL_onePoleFilter(vector, coefficient, UTL_head(vector))
              : let(next = UTL_head(vector) * (1 - coefficient) +
                           previous * coefficient,
                    remainder = UTL_tail(vector))
                    concat(next,
                           UTL_onePoleFilter(remainder, coefficient, next));
